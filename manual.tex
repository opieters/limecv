...
\section{Requirements}
  ...
  It is advised to use the \cvRequirement{XeLaTeX} compiler. However, there is also support for \cvRequirement{LaTeX} and \cvRequirement{LuaLaTeX}. In these cases, fonts might be different than intended and side bearings will not be corrected, since only the XeLaTeX engine has access to these. In the subsequent sections, it will always be assumed that the XeLaTeX compiler is used (unless noted otherwise). 
...
\begin{itemize}
  ...
  \item |\cvMainWidth| Width of the main content section. Default value: |\paperwidth-4\cvMargin-\cvSideWidth|.
